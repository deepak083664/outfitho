    ...
    const fetchProduct = async () => {
      try {
        ...
        if (data.images && data.images.length > 0) {
          initialImages = data.images.map(url => ({ url, preview: url, isExisting: true }));
        } else if (data.image) {
          initialImages = [{ url: data.image, preview: data.image, isExisting: true }];
        }
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-20 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8 px-2">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold font-heading text-gray-900">Edit Product</h1>
            <p className="text-gray-500 text-xs md:text-sm mt-0.5 tracking-wide">Update product details and inventory</p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-1 focus:ring-black focus:border-black appearance-none bg-white transition-shadow"
                >
